...
namespace bnb::gl
{
    ...
    bool context_info::is_rgba16f_available()
    {
        return true;
    }

    const char* context_info::error_code_to_string(GLenum error_code) const
    {
        switch (error_code) {
            // clang-format off
    ...
    void context_info::on_error(GLenum error_code, const char* file, int line)
    {
        WRITE_LOG_MESSAGE(
            warning, "glGetError: " << error_code_to_string(error_code) << " | " << file << " (" << line << ") ");
    }

    void context_info::check_error(const char* file, int line)
    {
        GLenum error_code;
